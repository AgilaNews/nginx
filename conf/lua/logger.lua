...
local function _unpack_i32_be(data)
...
local send_buffer           = ""
-- number of log lines in 2nd level buffer
...
local function _prepare_stream_buffer()
    ...
    send_buffer_count = send_buffer_count + log_buffer_index
...
local function _do_flush()
    local ok, err, sock, bytes, data, partial
    local packet = send_buffer
    local count = send_buffer_count
    ...
    data, err, partial = sock:receive(4)
    if not data then
        return nil, err
    end
